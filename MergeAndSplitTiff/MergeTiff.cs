...
namespace MergeAndSplitTiff
{
    public class MergeTiff
    {
        ...
        /// <param name="filePath">tif文件1</param>  
        /// <param name="targetFile">tif文件2</param>  
        public void AppendToTiffParallel(string filePath, string targetFile, long limitSize)
        {
            ArrayList list = new ArrayList(); //保存所有 tif文件路径  
            ArrayList list1 = new ArrayList();
            ArrayList list2 = new ArrayList();
            ...
            File.Delete(targetFile);
            //2.2 拼接 并按原路径生成tif文件  
            MergeTiffImagesWithLimitSize(list, targetFile, EncoderValue.CompressionCCITT3, limitSize);
            //3. 删除临时目录  
            DirectoryInfo di2 = new DirectoryInfo(tempDirectory2);
            di2.Delete(true);
            DirectoryInfo di1 = new DirectoryInfo(tempDirectory1);
        ...
        /// <param name="outFile">拼接后保存的 tif文件路径</param>  
        /// <param name="compressEncoder">压缩方式</param> 
        /// <param name="limitSize">文件大小限制 单位M</param>
        public void MergeTiffImagesWithLimitSize(ArrayList imageFiles, string outFile, EncoderValue compressEncoder, long limitSize)
        {
            int clipStartPageNumber = 0;
            ArrayList clipsFiles = new ArrayList();
            float mergeSize = 0f;
            float fileSize = 0f;
            foreach (string itemFile in imageFiles)
            {
                FileInfo size = new FileInfo(itemFile);
                mergeSize += size.Length;

               fileSize = mergeSize / (float)(1024 * 1024);

                if (fileSize > limitSize) {

                    clipStartPageNumber = imageFiles.IndexOf(itemFile);
                    clipsFiles = (ArrayList)imageFiles.GetRange(clipStartPageNumber, imageFiles.Count - clipStartPageNumber).Clone();
                    imageFiles.RemoveRange(clipStartPageNumber, imageFiles.Count - clipStartPageNumber);
                    string fileNameClip = outFile;
                    if (fileNameClip.Contains("Part"))
                    {
                        Regex regex = new Regex("_\\d+_");
                        Match match = regex.Match(outFile);
                        string[] matchValue = match.Value.Split('_');

                        int partNumber = Convert.ToInt32(matchValue[1]);
                        fileNameClip = outFile.Substring(0, outFile.IndexOf(match.Value)) + "_" + (partNumber + 1) + "_" + outFile.Substring(outFile.LastIndexOf("."));
                    }
                    else
                    {
                        fileNameClip = outFile.Substring(0, outFile.LastIndexOf(".")) + "Part_0_" + outFile.Substring(outFile.LastIndexOf("."));
